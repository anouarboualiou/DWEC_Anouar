...
const texto = document.getElementById('textoVisita');
const cerrarBtn = document.getElementById('cerrarBanner');

const ultimaVisita = leerCookie('ultimaVisita');

if (ultimaVisita) {
  texto.textContent = `Bienvenido de nuevo. Tu última visita fue el ${ultimaVisita}`;
...
function guardarUltimaVisita() {
  ...
  document.cookie =
    `ultimaVisita=${fecha}; expires=${expiracion.toUTCString()}; path=/`;
}
