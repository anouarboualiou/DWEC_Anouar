...
function getByArtistaId (artistaId){
    return albumes.filter(album => album.artistaId == artistaId)
}

function save(data){
    if(data.id){
        const index = albumes.findIndex(a => a.id == data.id)
        ...
        const newId = albumes.length > 0 ? Math.max(...albumes.map(a => a.id)) + 1 : 1
        data.id = newId
        albumes.push(data)
...
function deleteAlbum(id){
    ...
    albumes= albumes.filter(album => album.id != id)
}

module.exports = {getAll, getById, getByArtistaId, save, deleteAlbum}